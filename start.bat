...
echo [DEBUG] Starting OpenSSL detection...
echo.

echo [*] XForce Terminal - Starting...
echo.
...
echo [*] ============================================
echo [*] Building Components (Incremental)
echo [*] ============================================
...
if !BACKEND_BUILD_EXIT_CODE! NEQ 0 (
    ...
    if exist target\release\backend.exe (
        ...
        echo [DEBUG] Backend build failed - checking for existing binary...
        echo [DEBUG] Checking: target\release\backend.exe
        if exist target\release\backend.exe (
            echo [DEBUG] Existing binary found
        ) else (
            echo [DEBUG] No existing binary found
        )
        ...
        echo.
        echo [DEBUG] OpenSSL detection status:
        echo [DEBUG]   OPENSSL_FOUND: %OPENSSL_FOUND%
        echo [DEBUG]   OPENSSL_PATH: !OPENSSL_PATH!
        echo [DEBUG]   OPENSSL_METHOD: !OPENSSL_METHOD!
        echo.
        echo [*] To fix this issue:
        echo.
        echo     1. Run install-openssl.bat (recommended)
        echo        This script will guide you through installation.
        echo.
        echo     2. Or install manually:
        echo        - Chocolatey: choco install openssl
        echo        - Manual: Download from https://slproweb.com/products/Win32OpenSSL.html
        echo        - vcpkg: vcpkg install openssl:x64-windows
        echo.
        echo     3. After installation:
        echo        - RESTART YOUR TERMINAL (required)
        echo        - Run start.bat again
        echo.
        echo [*] For detailed installation instructions, run:
        echo     install-openssl.bat
        echo.
        exit /b 1
    )
) else (
...
if !TERMINAL_BUILD_EXIT_CODE! NEQ 0 (
    ...
    echo ============================================
    echo.
    echo [DEBUG] Terminal build failed - checking details...
    echo [DEBUG] Build exit code: !TERMINAL_BUILD_EXIT_CODE!
    echo [DEBUG] OpenSSL detection status:
    echo [DEBUG]   OPENSSL_FOUND: %OPENSSL_FOUND%
    echo [DEBUG]   OPENSSL_PATH: !OPENSSL_PATH!
    echo [DEBUG]   OPENSSL_METHOD: !OPENSSL_METHOD!
    ...
    echo [ERROR] The Solana SDK requires OpenSSL on Windows.
    echo.
    echo [*] Backend will still start - you can run terminal separately later.
    echo [*] However, the GUI application requires OpenSSL to be installed.
    echo.
    echo [*] To fix this issue:
    echo.
    echo     1. Run install-openssl.bat (recommended)
...
if defined BACKEND_BUILD_FAILED (
    if %BACKEND_BUILD_FAILED% EQU 0 (
        echo [DEBUG]   Backend: SUCCESS
    ) else (
        echo [DEBUG]   Backend: FAILED - exit code: !BACKEND_BUILD_EXIT_CODE!
    )
) else (
    echo [DEBUG]   Backend: Not attempted
)
if defined TERMINAL_BUILD_FAILED (
    if %TERMINAL_BUILD_FAILED% EQU 0 (
        echo [DEBUG]   Terminal: SUCCESS
    ) else (
        echo [DEBUG]   Terminal: FAILED - exit code: !TERMINAL_BUILD_EXIT_CODE!
    )
) else (
    echo [DEBUG]   Terminal: Not attempted
)
if defined WALLET_BUILD_EXIT_CODE (
    if !WALLET_BUILD_EXIT_CODE! EQU 0 (
        echo [DEBUG]   Wallet-web: SUCCESS
    ) else (
        echo [DEBUG]   Wallet-web: FAILED - exit code: !WALLET_BUILD_EXIT_CODE!
    )
) else (
    echo [DEBUG]   Wallet-web: Not attempted
)
echo [DEBUG]   Trunk available: %TRUNK_AVAILABLE%
echo.
...
if exist wallet-web\dist (
    ...
    echo cd /d wallet-web >> temp_wallet.bat
...
:START_TERMINAL
echo.
echo [3/3] Starting GUI application...
echo.
...
if !VERIFY_RETRY! LSS 10 (
    echo [*] Verifying... !VERIFY_RETRY!/10
    timeout /t 1 /nobreak >nul 2>&1
    goto VERIFY_BACKEND
)
...
if "%TERMINAL_BUILD_FAILED%"=="1" (
    ...
    echo [*] Starting terminal with latest build...
